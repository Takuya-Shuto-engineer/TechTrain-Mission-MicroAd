...
class HelloController @Inject()(cc: ControllerComponents) extends AbstractController(cc) {
  ...
  def hello(): Action[AnyContent] = Action{
    request => {
      val uuid: String = request.cookies.get("userId").map(_.value).getOrElse(java.util.UUID.randomUUID.toString)
      val count: String = request.cookies.get("count").map(_.value).getOrElse("1")
